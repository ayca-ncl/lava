...
@requires(CPU)
@tag('graded')
class PyLifModelBitAccGraded(PyLoihiProcessModel):
    ...
    def __init__(self):
        super(PyLifModelBitAccGraded, self).__init__()
    ...
    def run_spk(self):
        ...
        # ----------------------------
        s_out_inx = self.v >= self.effective_vth
        s_out = self.v * (self.v >= self.effective_vth)
        #print("SOOOOUT", s_out)
...
@implements(proc=LIF, protocol=LoihiProtocol)
@requires(CPU)
@tag('bit_accurate_loihi', 'fixed_pt', 'graded')
